...
- **liCHi-C** - low-input capture Hi-C techonology. In contrast to promoter-capture Hi-C (PCHiC) that requires 30-50M cells, works with as low as 50K cells, primary samples, total cost is approx \$1500. Processing with HiCUP, CHiCAGO. Benchmarked against other 3D technologies (Low-C, Hi-C with 4- and 5-cutter restriction enzymes, TagHi-C). Applied to normal and malignant human hematopoietic cells, promoter interactome rewiring, disease gene prioritization (Bayesian, only about 20% associated with nearest gene, 38% associated with multiple genes), enrichment in histone marks. Can detect structural variants (translocations, CNVs, topological structures). Scripts for all analyses on [GitHub](https://github.com/JavierreLab/liCHiC). <details>
  <summary>Paper</summary>
  Tomás-Daza, Laureano, Llorenç Rovirosa, Paula López-Martí, Andrea Nieto-Aliseda, François Serra, Ainoa Planas-Riverola, Oscar Molina, et al. “Low Input Capture Hi-C (LiCHi-C) Identifies Promoter-Enhancer Interactions at High-Resolution.” Preprint. Genomics, November 5, 2022. https://doi.org/10.1101/2022.11.04.515239.
</details>
