...
- <a name="htad">[HTAD](https://github.com/shenscore/HTAD)</a> - human-guided TAD detection, Discriminative Larning Approach (classification) on human-labeled TADs using horisontal/vertical border strength, vertex area, diamond score (Multi-Layer Perceptron, Tensorflow). Initial TAD calls using a simplified Directionality Index (sDI, increased sensitivity, decreased accuracy). Outperforms 3DNetMod, HiCexplorer, TopDom, Arrowhead, rGMAP on GM12878 Hi-C, calls hierarchical TADs on simulated data. Robust to resolutions. Observations about relationship between TAD structures and histone marks. CTCF strength proportional to TAD order (outer TADs have more CTCF, more conserved sequence). Input - .cool file. Manually annotated TADs on [Zenodo](https://zenodo.org/records/14186235), hg19, bin numbers at 10kb resolution. <details>
  <summary>Paper</summary>
  Shen, Wei, Ping Zhang, Yiwei Jiang, Hailin Tao, Zhike Zi, and Li Li. “HTAD: A Human-in-the-Loop Framework for Supervised Chromatin Domain Detection.” Genome Biology 25, no. 1 (2024): 302. https://doi.org/10.1186/s13059-024-03445-x.
</details>
...
- [TAD cliques](https://github.com/robomics/call_tad_cliques) - long-range interactions among TADs. Domains that form large TAD cliques tend to be repressive across cell types, when comparing gene expression, LINE/SINE repeat content and chromatin subcompartments (6 subcompartments). Further, TADs in large cliques are larger in genomic size, less dense and depleted of convergent CTCF motifs, in contrast to smaller and denser TADs formed by a loop extrusion process. Not much effect on gene expression. HMEC, HUVEC, IMR90, K562 cell lines. The Bron–Kerbosch algorithm - graph-theoretic algorithm for enumerating all maximal cliques in an undirected graph — that is, it finds every subset of vertices where every pair of vertices is connected, and such that you cannot add any more vertices without breaking that property. TADs called by Armatus. [GitHub](https://github.com/robomics/call_tad_cliques). <details>
  <summary>Paper</summary>
  Liyakat Ali, Tharvesh M., Annaël Brunet, Philippe Collas, and Jonas Paulsen. “TAD Cliques Predict Key Features of Chromatin Organization.” BMC Genomics 22, no. 1 (2021): 499. https://doi.org/10.1186/s12864-021-07815-8.
</details>
...
- [Hi-Compass](https://github.com/EndeavourSyc/Hi-Compass) - depth-aware deep learning framework predicting cell-type-specific Hi-C contact maps at 10 kb resolution from (sc)ATAC-seq as the sole cell-type-aware input. CNN encoders for DNA sequence, ATAC-seq signal, and a generalized CTCF profile; a depth-aware module accommodates sequencing depths from sparse single-cell to bulk; a meta-cell strategy handles single-cell and spatial sparsity. Outperforms Akita, C.Origami, Epiphany, ChromaFold, and AlphaGenome on insulation-score correlation and SSIM, recovers high-confidence loops, links GWAS variants to target genes, and transfers to mouse by fine-tuning. Pre-built hg38 and mm10 references, `pip install hicompass`. Output: balanced .cool compatible with cooltools, HiGlass, Juicebox. <details>
  <summary>Paper</summary>
  Sun, Yuan-Chen, Wen-Jie Jiang, Kang-Wen Cai, et al. "Hi-Compass: A Depth-Aware Deep Learning Framework for Predicting Cell-Type-Specific 3D Genome Organization from Single-Cell to Spatial Resolution." Nature Communications 17, 5172 (2026). https://doi.org/10.1038/s41467-026-71877-z
</details>
...
- [CREATE](https://github.com/cuixj19/CREATE) - deep learning integrating sequences (one-hot encoded), accessibility, chromatin interaction (loop count) data (omics-specific encoders, embeddings concatenated) for CRE identification, multimodal CNN leveraging Vector Quantized (replaces the output embeddings with their nearest neighbors in the codebook, created during training (K=50-800, default 200 codebook size, M number of splits of embedding vectors), interpretable discrete embeddings) Variational AutoEncoder, then decoder of the original multi-omics inputs and the classifier module. Different models trained on individual omics, pairs of omics, and all three, the full model performing best. Outperforms baseline methods (DeepSEA, DanQ, ES-transition, DeepICSH, CREATE). Predicts sliencers, enhancers, promoters, insulators, compares enrichment in epigenomic features, conservation. <details>
  <summary>Paper</summary>
  Cui, Xuejian, Qijin Yin, Zijing Gao, et al. “CREATE: Cell-Type-Specific Cis-Regulatory Element Identification via Discrete Embedding.” Nature Communications 16, no. 1 (2025): 4607. https://doi.org/10.1038/s41467-025-59780-5.
</details>

- <a name="cdachie">[CDACHIE](https://github.com/maruyama-lab-design/CDACHIE)</a> - annotating chromatin domains by clustering (k-means, k=6) chromatin interaction (Hi-C, graph, the LINE node embedding method (Large-scale information network embedding), 128-dimensional embeddings) and epigenomic data (16-dimensional embeddings). Bins of 100kb. Two encoders: structural and functional. 6 transformer blocks. contrastive learning - maximizing cosine similarity between same bin vectors and minimizing between different. Compared with HMM_combined, Segway GMM_GRB, and Rao 2014 subcompartments. GM12878 and K562. Detected domains (C1-C6) evaluated using variance explained gene expression, replication timing, CTCF, RNAPII. Projecting concatenated embeddings on 2D UMAP and coloring by histone modifications. Extends/follows <a href="#ichda">IChDA</a> work. <details>
  <summary>Paper</summary>
...
- <a name="ichda">[IChDA](https://github.com/nedashokraneh/IChDA)</a> - chromatin domain annotation (8 types) using Hi-C structural embeddings (LINE node embedding model, second-order node proximity) and epigenomic tracks (signals), segmentation and genome annotation (SAGA) clustering algorithm. Evaluated using percentage of variance explained, gene expression, replication timing, alignment with ChIA-PET tracks. <details>
  <summary>Paper</summary>
  Shokraneh, Neda, Mariam Arab, and Maxwell Libbrecht. “Integrative Chromatin Domain Annotation through Graph Embedding of Hi-C Data.” Bioinformatics 39, no. 1 (2023): btac813. https://doi.org/10.1093/bioinformatics/btac813.
</details>
